...
try:
    import cv2
except Exception:  # pragma: no cover - optional for server deployment
    cv2 = None


def capture_preview_camera():
    """Desktop-only camera utility. The web server never calls this on Render."""
    if cv2 is None:
        raise RuntimeError("OpenCV is not installed in this environment.")

    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        raise RuntimeError("Could not open local camera capture.")
    ...
    try:
        while True:
            ...
            if rec_visible:
                ...
                cv2.putText(frame, "REC", (55, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
